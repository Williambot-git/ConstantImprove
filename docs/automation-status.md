...
- **test(backend): add logger.js unit tests** (20 cases, 100% coverage) — `backend/tests/utils/logger.test.js` covers: LOG_LEVEL env filtering (error/warn/info/debug, including unknown-level fallback), NODE_ENV defaults (production=info, development/debug=debug), output routing (console.error→stderr, console.warn→stderr, console.log→stdout), formatMessage (ISO timestamp, uppercase level, JSON meta, empty-meta omission). Uses subprocess isolation to test real module behavior under controlled env.
- **test(backend): add jwt utils unit tests** (17 cases, 100% line/branch/function coverage) — `backend/tests/utils/jwt.test.js` covers all 5 exported functions. **2,051 total tests passing** (1,236 backend + 815 frontend). 40 backend suites, 48 frontend suites — all passing.
