...
- **test(backend): add jwt utils unit tests** (17 cases, 100% line/branch/function coverage) — `backend/tests/utils/jwt.test.js` covers all 5 exported functions: `generateToken` (signs with JWT_SECRET, 15m default, custom options), `generateRefreshToken` (signs with REFRESH_SECRET, 7d default), `verifyToken` (returns payload or null on invalid/expired/error), `verifyRefreshToken` (same pattern with refresh secret), `decodeToken` (passthrough to jwt.decode). **2,031 total tests passing** (1,216 backend + 815 frontend). 39 backend suites, 48 frontend suites — all passing.
