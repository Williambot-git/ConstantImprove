 ...
 * @param {string} accountNumber    - VPN account number (used in username prefix)
...
async function createVpnAccount(userId, accountNumber, planInterval, { renew = false } = {}) {
  ...
  if (renew) {
    const existing = await db.query(
      'SELECT id, vpn_uuid, vpn_username, vpn_password FROM vpn_accounts WHERE user_id = $1',
      [userId]
    );
    if (existing.rows.length > 0) {
      ...
      const { vpn_uuid: existingUuid } = existing.rows[0];
      if (existingUuid) {
        try {
      ...
      return {
        username: updated.rows[0]?.vpn_username,
        password: updated.rows[0]?.vpn_password,   // unchanged — kept from original creation
  ...
  const result = await db.query(
    `INSERT INTO vpn_accounts (user_id, vpn_username, vpn_password, vpn_uuid, expiry_date, status, allowed_countries, created_at, updated_at)
