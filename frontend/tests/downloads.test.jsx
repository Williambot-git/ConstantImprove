 ...
 * ==========================
 * Tests the VPN client download page (platforms: Windows, macOS, Linux, Android, iOS, Firestick).
 *
 * WHY THIS TEST:
 * - Static content page with DOWNLOADS constant array
 * - No auth, no API calls, no complex state
 * - Smoke test ensures page renders all 6 platforms
 * - First line of coverage for this 198-line page
 */
...
describe('downloads.jsx', () => {
  ...
  it('renders all 6 platform download cards', () => {
    render(<DownloadsPage />);
    // Platform name is in an h3 element inside each Card.
    // There are 7 h3 elements total: 6 for platforms + 1 for "Want a router config?"
    ...
    expect(platformNames.length).toBe(6);
  });
  ...
  it('renders download button text for each platform', () => {
    ...
    expect(screen.getByText('Download .run')).toBeInTheDocument();
  ...
  it('renders correct href links for each platform', () => {
    ...
    expect(hrefs).toContain('https://vpnclient.app/current/vpnclient/vpnclient.run');
